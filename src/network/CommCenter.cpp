...
#if defined(LINUX_PORT) && !defined(_WIN32)
#include "network/CommCenterLinuxSStubs.h"
#else
#include "T4CLog.h"
...
#if !defined(USE_CLIENT_CONNECTION) && defined(LINUX_PORT) && !defined(_WIN32)
...
#ifndef USE_CLIENT_CONNECTION
   #define NM_WAIT_IO  60000 // BLBLBL 10/12/2010 60000, je baisse ? 5000 comme pour le serveur, pour voir//13/12/2010 ?a change rien car cette ligne c'est pour le serveur..
#else
...
{
    const unsigned long a1 = static_cast<unsigned long>(sock1.sin_addr.s_addr);
    const unsigned long a2 = static_cast<unsigned long>(sock2.sin_addr.s_addr);
    if (a1 != a2) {
        return a1 < a2;
    }
...
CCommCenter::CCommCenter( CCommMonitor *lpMon)
{
   ...
   sRecvSocket = INVALID_SOCKET;
   sSendSocket = INVALID_SOCKET;
...
#ifndef USE_CLIENT_CONNECTION
   #define WAIT_THREAD( __thread ) if( WaitForSingleObject( __thread, 5000 ) == 258u ) {\
                                    TerminateThread( __thread, 1 );\
                                }
#else
   #define WAIT_THREAD( __thread ) if( WaitForSingleObject( __thread, 500 ) == 258u ) {\
                                    TerminateThread( __thread, 1 );\
                                }
#endif
...
#if defined(LINUX_PORT) && !defined(_WIN32)
         T4CNetworkDebugLog("[NET] InitSocket: bound UDP, local port %u",
...
/*      #ifdef USE_CLIENT_CONNECTION //BLBLBL 08/12/2010 ??? un thread ? la con ?_
...
{
   //OutputDebugString("SendLock LOCK\n");
   m_SendLock.Lock();
   CNMConnection* lpConnection = NULL;
   lpConnection = GetConnection( sockAddr );

   if(nBufferSize <= MAX_DATA_SIZE) //None splitted packet...
   {
       ...
       PostPreSendData(pPacket);
          ...
          if(m_bSendCRC)
          {
             ...
             //Send the packet to Pre Sending queue
             //PostPreSendData(pPacket2); // steph d?sactivation
             m_bSendCRC = FALSE;
          }
      #endif
   }
   ...
   m_SendLock.Unlock();
   //OutputDebugString("SendLock UNLOCK\n");
}

void CCommCenter::ReSendPacket(UDPPacket* pPacket)
{
   //OutputDebugString("RE-SendLock LOCK\n");
   m_ReSendLock.Lock();//BLBLBL 09/12/2010 ajout d'un lock ici
   ...
   pNewSendPack->packetHeader->safe     = pPacket->packetHeader->safe;    
   ...
   m_ReSendLock.Unlock();
   //OutputDebugString("Re-SendLock UNLOCK\n");
}
...
void CCommCenter::UDPReceiveDataThread(LPVOID lpData)
{
      ...
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
         "CommCenter::UDPReceiveDataThread Id=%u",
         GetCurrentThreadId()
         LOG_
   #endif
   ...
   while( lpComm->bUDPReceiveDataThread )
   {
	 ...
	 // OutputDebugString("UDPReceiveDataThread : OK\n");//BLBLBL 10/12/2010
      ...
      if( pPacket->nBufferLen < 0 /*|| rand() % 100<5*/)//BLBL simulation mauvaise connexion o? des paquets sont perdus.
      {
#if defined(LINUX_PORT) && !defined(_WIN32)
         T4CNetworkDebugLogKind(T4CMatrixLogKind::Warn, "[UDP] recvfrom erreur ret=%d errno=%d (%s)", pPacket->nBufferLen,
                               errno, std::strerror(errno));
#endif
         delete pPacket;
		 OutputDebugString(" ++ deleted a network packet which was empty\n");
      }
      else
      {
         ...
         if( lpComm->bUDPReceiveDataThread )
         {
...
#if defined(LINUX_PORT) && !defined(_WIN32)
               T4CNetworkDebugLog("[UDP] <- recvfrom raw %d bytes (queued to packet thread)", pPacket->nBufferLen);
#endif
            ...
            else if(pPacket->nBufferLen == 1)
            {
               if( lpBuffer[ 0 ] == 0xF1 )
               {
                  ...
                  T4CNetworkDebugLogKind(T4CMatrixLogKind::Warn, "[UDP] 1 octet recu 0x%02X (pas 0xF1), ignore",
                                          static_cast<unsigned>(lpBuffer[0]));
                  delete pPacket;
               }
#else
               else {
            ...
            else
            {
               ...
               T4CNetworkDebugLogKind(T4CMatrixLogKind::Warn,
                                       "[UDP] datagramme ignore len=%d (attendu %d<=len<=%d ou 1 octet 0xF1)",
                                       pPacket->nBufferLen, HEADER_SIZE, MAX_PACKET_SIZE);
#endif
               delete pPacket;
            }
         }
...
void CCommCenter::UDPSendDataThread(LPVOID lpData)
{
      ...
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
         "CommCenter::UDPSendDataThread Id=%u",
         GetCurrentThreadId()
         LOG_
   #endif
   ...
   {
	  //OutputDebugString("UDPSendDataThread : OK\n");//BLBLBL 10/12/2010
      
      DWORD dwFoo               = 0;
      ULONG_PTR dwPacketAddr    = 0;
      ...
      if(bRet)
      {
         ...
         if( !pPacket->boDelete)
         {
            ...
            // Send message.
            const int sendRet = sendto(lpComm->sSendSocket, (char *)pPacket->lpBuffer, pPacket->nBufferLen, 0,
                                       (sockaddr *)&pPacket->sockAddr, sizeof( sockaddr_in ) );
            if( sendRet == SOCKET_ERROR)
            {
               ...
               T4CNetworkDebugLog("[UDP] -> sendto OK, %d bytes (UDP frame)", sendRet);
            }
#endif
         }
         #ifndef USE_CLIENT_CONNECTION
         ...
         {
            // Sets the time when the packet will need to be resent.
...
void CCommCenter::UDPMaintenancePacket(LPVOID lpData)
{
      ...
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
         "CommCenter::UDPMaintenancePacket Id=%u",
         GetCurrentThreadId()
         LOG_
   #endif
   ...
   while( lpComm->bUDPMaintenancePacket )
   {
	  ...
	  //OutputDebugString("UDPMaintenancePacketThread : OK\n");//BLBLBL 10/12/2010
...
void CCommCenter::UDPReceivePacketThread(LPVOID lpData)
{
      ...
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
         "UDPReceivePacketThread Thread Id=%u",
         GetCurrentThreadId()
         LOG_
   #endif
   ...
   const int MaxWaitCount = 10; // DaP ajout de int
   int   nWaitCount   = 0;
   while( lpComm->bUDPReceivePacketThread )
   {

	  //OutputDebugString("UDPReceivePacketThread : OK\n");//BLBLBL 10/12/2010      
...
void CCommCenter::UDPAnalyseThread(LPVOID lpData)
{
   ...
   while( lpComm->bUDPAnalyseThread )
   {
	  ...
	  //OutputDebugString("UDPAnalyseThread : OK\n");//BLBLBL 10/12/2010
      ...
      BOOL bRet = FALSE;
         ...
         if(lpComm->m_bIsWindowNT)
      #endif
            bRet = GetQueuedCompletionStatus( lpComm->hhUDPAnalyseIO, &dwFoo, &dwPacketAddr, &lpOverlapped, NM_WAIT_IO );
      #ifdef USE_CLIENT_CONNECTION
         else
         {
...
#if defined(LINUX_PORT) && !defined(_WIN32)
         T4CNetworkDebugLog("[NET] UDPAnalyseThread -> lpReadCallback (%d app bytes)", pPacket->dataLen);
#endif
...
//////////////////////////////////////////////////////////////////////////////////////////
   ...
   {
      return;//NMNMNM //BLBLBL 08/12/2010 ??? c'est quoi ce bordel de fonction pour crash le client ?
...
void CCommCenter::UDPSendPacketThread(LPVOID lpData)
{
      ...
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
         "UDPSendPacketThread Thread Id=%u",
         GetCurrentThreadId()
         LOG_
   #endif
   ...
   {
	  //OutputDebugString("UDPSendPacketThread : OK\n");//BLBLBL 10/12/2010
      ...
      if(bRet)
      {
         ...
         if(pPacket->packetHeader->Reserved)
            dwNbr0 = 0;
         else
            dwNbr0++;
		 ...
		 //BLBLBL on stocke le header du packet avant le cryptage (qui modifie le header)
		 pPacket->HeaderSave=*pPacket->packetHeader;
		 //pPacket->needAck = pPacket->packetHeader->safe;
		 //pPacket->uncryptedPacketID = pPacket->packetHeader->packetID;
         ...
         //if (pPacket->packetHeader->safe ) //BLBLBL Bug spotted par Chaotik, ? cet instant pPacket n'est plus valable ? cause de la fonction Encrypt qui modifie le pointeur.
         if (pPacket->HeaderSave.safe) 
		 //if (pPacket->needAck)//BLBLBL tentative de simplification
		 {
            lpConnection->AddPending(pPacket);
			//OutputDebugString("Packet need ack, so AddPending packet\n");//BLBLBL 13/12/2010
         }
         lpComm->cConnectionLock.Unlock();
...
{ 
...
#if defined(LINUX_PORT) && !defined(_WIN32)
      if(uiCheckPacket != 0){
         T4CNetworkDebugLogKind(T4CMatrixLogKind::Warn, "[UDP] DecryptS echec nBufferLen=%d", pPacket->nBufferLen);
      }
#endif
   ...
   if(uiCheckPacket != 0) //Checksum invalide or unable to uncrypt packet...
   {
      ...
      delete pPacket;
      pPacket = NULL;
	  //OutputDebugString("UI Check error on packet...\n");//BLBLBL 13/12/2010
      return;
   }

   /* En-t?te apr?s DecryptS : m?me encodage 16-bit que MSVC (serveur Windows / binaire d?origine).
    * GCC interpr?te les bitfields diff?remment : ne pas utiliser packetHeader pour l?analyse entrante. */
   const unsigned in_raw =
       static_cast<unsigned>(pPacket->lpBuffer[0]) | (static_cast<unsigned>(pPacket->lpBuffer[1]) << 8);
   const unsigned in_safe = in_raw & 1u;
   const unsigned in_lastFrag = (in_raw >> 1) & 1u;
   const unsigned in_reserved = (in_raw >> 2) & 1u;
   const unsigned in_packetID = (in_raw >> 3) & 0x1FFFu;

#if defined(LINUX_PORT) && !defined(_WIN32)
      T4CNetworkDebugLog(
          "[UDP] post-decrypt hdr raw=0x%04X safe=%u lastFrag=%u rsv=%u pktID=%u len=%d dataLen=%d",
          in_raw & 0xFFFFu,
          in_safe,
          in_lastFrag,
          in_reserved,
          in_packetID,
          pPacket->nBufferLen, pPacket->dataLen);
#endif
      
   //////////////////////////////////////////////////////////////////////////////////////////        
   // If packet is an ack (acks are only header, no data!)
   // BLBLBL if we receive an ack (searching RQ_ACK ? look here :)
   if( pPacket->nBufferLen == HEADER_SIZE  && ((in_lastFrag + in_safe) == 0 ))
   {
      ...
      lpConnection->DestroyPending(static_cast<WORD>(in_packetID));
	  //OutputDebugString("Destoying packet...\n");//BLBLBL 13/12/2010
   ...
   else  // If packet isn't an ack.
   {
      ...
      else*/ if( lpConnection->AlreadyReceivedPacket(static_cast<WORD>(in_packetID)) == false )
      {
         // Register the packet on the list of received packets
         lpConnection->RegisterReceivedPacketID(static_cast<WORD>(in_packetID));
         
         //////////////////////////////////////////////////////////////////////////////////////////
         // If this is a packet fragment.
         if( in_packetID < NONFRAGMENTED_PACKETS_IDOFFSET  )
         {
            // only last fragments are allowed to have size smaller than max
            if( in_lastFrag == 1 || pPacket->dataLen == MAX_DATA_SIZE)
            {
               //char strTmp[100];
   ...
   {
      return;//NMNMNM//BLBLBL vir? cette merde.
...
inline void CCommCenter::PostReceivePacket (UDPPacket* pPacket)
{
      ...
      if(m_bIsWindowNT)
   #endif
         PostQueuedCompletionStatus( hhUDPReceivePacketIO, 0, reinterpret_cast< ULONG_PTR >( pPacket ), NULL );
   #ifdef USE_CLIENT_CONNECTION
      else
...
inline void CCommCenter::PostSendPacket (UDPPacket* pPacket) 
{
      ...
      if(m_bIsWindowNT)
   #endif
         PostQueuedCompletionStatus( hhUDPSendPacketIO, 0, reinterpret_cast< ULONG_PTR >( pPacket ), NULL );
   #ifdef USE_CLIENT_CONNECTION
      else
...
inline void CCommCenter::PostPreSendData (UDPPacket* pPacket)
{
      ...
      if(m_bIsWindowNT)
   #endif
         PostQueuedCompletionStatus( hhUDPPreSendPacketIO, 0, reinterpret_cast< ULONG_PTR >( pPacket ), NULL );
   #ifdef USE_CLIENT_CONNECTION
      else
...
inline void CCommCenter::PostAnalysePacket (UDPPacket* pPacket)
{
      ...
      if(m_bIsWindowNT)
   #endif
         PostQueuedCompletionStatus( hhUDPAnalyseIO, 0, reinterpret_cast< ULONG_PTR >( pPacket ), NULL );
   #ifdef USE_CLIENT_CONNECTION
      else
...
inline UDPPacket* FragmentsList::BuildCompletedPacket() 
{
   ...
   for (int i=0; i<MAX_FRAGMENTS_PER_PACKET; ++i) // DaP ajout de int
   {
      UDPPacket* pPacket = fragList[i];
		if (pPacket != NULL) 
      { // packet exists! Needa be destroyed!
...
inline void CNMConnection::AddPending(UDPPacket* pPacket) 
{
	...
	if (m_pendingPackets.size() < MAX_PACKET_PENDING_LIST) 
   {
      ...
      pPacketPend->packetHeader->safe     = pPacket->packetHeader->safe;    
	  ...
	  pPacketPend->HeaderSave=pPacket->HeaderSave;//BLBLBL Correctif de Chaotik pour pouvoir acc?der aux infos header apr?s cryptage.
...
{
   m_pendingLock.Lock();
   //OutputDebugString("DestroyPending.. BEGIN\n");//BLBLBL 13/12/2010
   std::list<UDPPacket*>::iterator i = m_pendingPackets.begin();//BLBLBL remplacement du vector par une std::list // std::list au lieu de std:list
	while (i != m_pendingPackets.end()) 
   {
		//OutputDebugString("Looking in the m_pendingPackets list...\n");//BLBLBL 13/12/2010
		//if ((*i)->packetHeader->packetID == packetID)
		if ((*i)->HeaderSave.packetID == packetID)//BLBLBL Correctif de chaotik pour que ce soit pas foireux ^^
      {
		 //BLBLBL ? voir si on entre effectivement dans ce bloc un jour, ? tester avec un breakpoint.
         UDPPacket* pPacket = (*i);
		 ...
		 //OutputDebugString("Removing packet type from m_pendingPackets\n");//BLBLBL 13/12/2010
   ...
   m_pendingLock.Unlock();
   //OutputDebugString("Destroy pending .. END\n");//BLBLBL 13/12/2010
	return false;
}
...
{
   m_pendingLock.Lock();
   std::list<UDPPacket*>::iterator i = m_pendingPackets.begin();//BLBLBL 06/12/2010 remplace par une list
	while ( i != m_pendingPackets.end() ) 
   {
      UDPPacket* pPacket = (*i);
...
inline void CNMConnection::VerifyTimedoutPending()
{
   ...
   m_pendingLock.Lock();
   std::list<UDPPacket*>::iterator i = m_pendingPackets.begin();///BLBLBL remplac? vector par une std::list 06/12/2010
   while ( i != m_pendingPackets.end() ) 
   {
      if ( /*(*i) != NULL && */(*i)->dwTimeout <= dwCurrentTime ) //BLBLBL ajout d'un test null//07/12/2010 je le vire
      {
         pResentPacket = *i;
         if(pResentPacket!=NULL) //BLBLBL ajout d'un test different de null
         {
            // It timed out, lets get it ready for retransmission!
            pResentPacket->dwTimeout = GetRunTime() + pResentPacket->dwAckDelay;//0xFFFFFFFF;//BLBLBL remplac? (*i) par pResentPacket et mis valeur correcte pour le delai
            //pResentPacket->dwTimeout = 0xFFFFFFFF;//BLBLBL pb de timeout je remet FFFFF pour voir
            ...
            if(pResentPacket->dwAckCount == 0)
            {
			   ...
			   //OutputDebugString("Deleted a pending packet, due to timeout.\n");//BLBLBL 13/12/2010

               if(pPackDel->lpBuffer)
                  delete []pPackDel->lpBuffer;
			...
			{/// 06/12/2010 : BLBLBL ajout de braces il ne faut pas envoyer un pak supprim?
              
            //Send the packet to Sending queue
            m_CommCenter->ReSendPacket(pResentPacket);
			 ++i;
			}
         }
         else
