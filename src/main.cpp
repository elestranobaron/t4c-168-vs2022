...
#include "XpStat.h" // shainon ajout
#include "PvpRanking.h" // asteryth ajout

#import "WebPatch.Exe" // DaP ajout
...
extern ClientConfig m_ClientConfig; // steph ajout de ClientConfig
...
PvpRanking pvpranking; // asteryth ajout
XpStat xpstat; // shainon ajout
	...
	while (!boMsgDay && SocketState == 1) 
	{
		...
		{		
			...
			if(Custom.NMDebug)
			{
				...
				for (int i = 0; i < nBufferSize2; i++) // DaP ajout de int
				{
					char Temp[100];
            ...
            case 91:
				{
					...
					Msg->Get((short *)&wLen);
					for (int i = 0; i < wLen; i++) // DaP ajout de int
						Msg->Get((char *)&Temp[i]);
					Temp[wLen] = 0;
					...
					*/
					...
					catch (_com_error e) 
					{
						...
						strcat(szDownloadDir, "\\download"); // steph download au lieu de Download
...
void InitializeTFCData(void) 
{
	...
	*/
	...
	// steph ajout
	//////////////////////////////////////////////////////////////////////////////////////////
	g_GUIDELocalString.Release();
	g_GUIDELocalString.Create(dwLanguage);
	...
	while(pLauncher->IsLuncherAvailable() && dwAction != 1)
	{
		...
		case LAST_CONNECT_ACTION_CONENCT: //case Connection...
            ...
            break;
				...
				case LAST_CHANGE_LANGUAGE:
					...
					//////////////////////////////////////////////////////////////////////////////////////////
					g_GUIDELocalString.Release();
					g_GUIDELocalString.Create(pLauncher->GetLanguage());
				...
				default :
					if(DlgStateTmp != DlgState)
					{
						...
						case 4:
							...
							HelpText2.Format(60);
							for(int i=0;i<HelpText2.GetnbLine();i++) // DaP ajout de int
								pLauncher->AddConnectMessage(HelpText2.GetText(i),i);
							break;
						case 6: 
     ...
     { 
		 ...
		 g_LocalString   .Release();
		 g_GUILocalString.Release();
		 g_GUIDELocalString.Release(); // steph ajout
		 exit(0);
     }
     ...
     LOG << "26, ";
     FILE *index = fopen("gamefiles\\MD.MD", "rb"); // steph gamefiles au lieu de Game Files
     ...
     else
     {
		 ...
		 //////////////////////////////////////////////////////////////////////////////////////////
		 hFile = fopen( "gamefiles\\T4CGameFile.VSB", "rb");
		 FILE *hFile2 = fopen( "gamefiles\\VSBInfo.Txt", "rb");
     ...
     if (Windowed)
     {
		 ...
		 SetWindowLong( siGethWnd(), GWL_STYLE, WS_CAPTION | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU); // Set titlebar, min/max/restore buttons and sysmenu // steph  WS_CAPTION | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU au lieu de WS_OVERLAPPEDWINDOW
     ...
     strcpy(FontName, "T4C BeaulieuxV2"); // steph T4C BeaulieuxV2 au lieu de T4C Beaulieux
	...
	// steph gamefiles au lieu de Game Files
	//////////////////////////////////////////////////////////////////////////////////////////
     World.SetWorldFileName   ("gamefiles\\V2_WorldMap.Map");
     World.SetWorldFileName   ("gamefiles\\V2_DungeonMap.Map");
     World.SetWorldFileName   ("gamefiles\\V2_CavernMap.Map");
     World.SetWorldFileName   ("gamefiles\\V2_Underworld.Map");
     ...
     */
     ...
     {
		 sprintf(strNameTmp,"Back%02d_%d",aa+1,g_Global.GetScreenH()); // steph H au lieu de W
		 Intro [aa].LoadSprite(strNameTmp);
     }
	 
     /* for(int aa=0;aa<36;aa++) // DaP ajout de int
     {
...
void ParseText(char *pText) 
{
	...
	{ 
		...
		// shainon ajout
		if (!_stricmp(szCmd, "!XPSTAT")) {
			xpstat.CalculateAndShowXpStat(Player.Exp,Player.Exp2Go);
		}
		else if(!_stricmp(szCmd, "!XPRESET") || !_stricmp(szCmd, "!XPSTART")) {
			xpstat.Init(Player.Exp);
		}
		...
		else if (!_stricmp(szCmd, "!Pvp"))
		{
			...
			else
			{
				...
				ChatterUI::GetInstance()->AddBackscrollItem("", "!Pvp [stat]", RGB(174,238,238), true);
...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window =
        SDL_CreateWindow("T4C (Linux SDL3)", 800, 600, SDL_WINDOW_RESIZABLE);
    if (!window) {
        SDL_Quit();
        return 1;
    }
